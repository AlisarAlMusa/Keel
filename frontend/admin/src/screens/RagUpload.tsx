...
export function RagUpload({ auth }: Props) {
  ...
  async function handleUpload() {
    ...
    setLoading(true);
    try {
      const result = await uploadDocument(auth, file, chunkType);
      setLastUpload({ filename: file.name, chunks: result.chunks_estimated });
      setToast({
        message: `Uploaded — ~${result.chunks_estimated} chunks queued for embedding`,
        kind: 'success',
      });
      setFile(null);
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto' }}>
      ...
      >
        Upload prose documents (catalog.md, policy.md, handbooks). Structured
        data like course listings is seeded separately.
      </p>
      ...
      <Card style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
        ...
        {lastUpload && (
          ...
          >
            ...
            ~{lastUpload.chunks} chunks queued
          </div>
        )}
      </Card>
