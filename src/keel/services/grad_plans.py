...
from keel.presenters.grad_card import build_grad_plan_card, term_label
...
async def _engine_context(
    ...
    *,
    ...
    Loads the raw student context from the repository, then maps it into engine
    objects via the shared mapper — no SQL and no tool-layer dependency here.
    """
    from keel.mappers.engine_context import _build_engine_objects
    from keel.repositories.students import StudentRepository

    data = await StudentRepository(session, tenant_id).load_context(str(student_id))
    if not data:
